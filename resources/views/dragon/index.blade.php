...
<div class="container game-container mt-5">
    ...
    <div class="row justify-content-center g-4" id="card-area">
        @foreach ([1 => 'Dragon', 2 => 'Tiger', 3 => 'Tie'] as $num => $label)
            @php
                $amount = collect($bets)->firstWhere('number', $num)->amount ?? 0;
                $imgUrl = $num == 1
                    ? 'https://demo.globalbet786.live/assets/dragon.png'
                    : ($num == 2
                        ? 'https://demo.globalbet786.live/assets/tiger.png'
                        : 'https://demo.globalbet786.live/assets/tie.png');
            @endphp
            <div class="col-6 col-sm-4 col-md-3 text-center">
                <div class="clickable-card card-image-full" 
                     style="background-image: url('{{ $imgUrl }}');" 
                     data-number="{{ $num }}">
                    <!--<div class="card-overlay">{{ $label }}</div>-->
                </div>
                <div class="amount-box" id="amount-{{ $num }}">₹{{ $amount }}</div>
            </div>
        @endforeach
    </div>
