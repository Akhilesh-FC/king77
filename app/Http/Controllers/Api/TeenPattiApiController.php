...
class TeenPattiApiController extends Controller
{
    ...
    foreach ($results as $result) {
        ...
        if ($win_amount > 0) {
            DB::table('users')->where('id', $result->userid)->increment('wallet', $win_amount);
        }
    }
    ...
    private function determineWinner($bankerCards, $playerCards)
{
    ...
    // 30% chance player win, 70% banker win
    $playerWins = rand(1, 100) <= 30; 

    if ($playerWins) {
        // Force player to win
        if ($playerRank <= $bankerRank) {
            // Agar player ka rank chhota ya equal hai, to uska rank artificially bada do
            $playerRank = $bankerRank + 1;
        }
        return 'player';
    } else {
        // Force banker to win
        if ($bankerRank <= $playerRank) {
            // Agar banker ka rank chhota ya equal hai, to uska rank artificially bada do
            $bankerRank = $playerRank + 1;
        }
        return 'banker';
    }
}
