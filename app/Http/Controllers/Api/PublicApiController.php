...
public function verifyAccountOtp(Request $request)
{
    ...
    $verifyUrl = "https://otp.fctechteam.org/verifyotp.php?mode=test&digit=4&mobile=$mobile&otp=$otp";
    ...
    $response = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim($response));
    curl_close($ch);
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        return response()->json([
            'message' => 'Invalid response from OTP server',
            'raw_response' => $response,
            'status' => 500
        ]);
    }

    if (isset($verifyData['error']) && $verifyData['error'] == "200") {
        // ✅ OTP verified, save session flag
    ...
    public function verifyOtpOnly(Request $request)
{
    ...
    $verifyUrl = "https://otp.fctechteam.org/verifyotp.php?mode=live&digit=4&mobile=$mobile&otp=$otp";
...
public function forgotPasswordSendOtp(Request $request)
{
    ...
    $url = "https://otp.fctechteam.org/send_otp.php?mode=live&digit=4&mobile=" . $mobile;
    ...
    if ($data['error'] == "200") {
        return response()->json([
            'otp' => $data['otp'], // ⚠️ Only for testing
            'message' => 'OTP sent successfully',
            'status' => '200'
        ], 200);
    } else {
...
public function verifyOtpAndUpdatePassword(Request $request)
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        return response()->json([
            'message' => 'OTP verification failed. Invalid response from OTP server.',
            'raw_response' => $response,
            'status' => '500'
        ]);
    }
    ...
    if ($data['error'] == "200") {
        return response()->json([
            'otp' => $data['otp'], // Only for testing; hide this in production
            'message' => 'OTP sent successfully',
            'status' => '200'
        ], 200);
    } else {
    ...
    public function login_with_Otps(Request $request)
    {
        $mobile = $request->input('mobile');
        $otp = $request->input('otp');
         $login_token = $request->input('login_token'); // ✅ Get token from request
    
         if (!$mobile || !$otp || !$login_token) {
        return response()->json([
            'message' => 'Mobile, OTP and login token are required',
            'status' => '400'
        ], 400);
    }
        ...
        // ✅ Prepare the cURL request for OTP verification
        $verifyUrl = "https://otp.fctechteam.org/verifyotp.php?mode=live&digit=4&mobile=$mobile&otp=$otp";
        ...
        // ✅ Remove BOM or invalid characters if present
        $response = trim($response);
        $response = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $response);
...
//Register
    public function sendOtp(Request $request)
    {
        ...
        $otpApiUrl = "https://otp.fctechteam.org/send_otp.php?mode=live&digit=4&mobile=" . $mobile;
        ...
        if (isset($responseData['error']) && $responseData['error'] == "200") {
            return response()->json([
                'status' => true,
                'message' => $responseData['msg'] ?? 'OTP sent successfully',
                'otp' => $responseData['otp'] ?? null,
            ]);
        } else {
            return response()->json([
                'status' => false,
                'message' => $responseData['msg'] ?? 'Failed to send OTP.',
            ]);
        }
    }
    ...
    public function verifyOtpAndRegister(Request $request)
{
    ...
    if (!$mobile || !$password) {
        ...
        return response()->json(['status' => false, 'message' => 'Session expired. Please try again.']);
    }
     //dd($password,$mobile);

    // ✅ OTP verify using curl
    $verifyUrl = "https://otp.fctechteam.org/verifyotp.php?mode=live&digit=4&mobile=$mobile&otp=$otp";
    ...
    $randomName = 'User_' . rand(1000, 9999);
    $uid = 'UID' . rand(10000, 99999);
    $randomReferralCode = strtoupper(Str::random(6));
    $baseUrl = URL::to('/');
    ...
    $userId = DB::table('users')->insertGetId([
        ...
        'u_id' => $uid,
        'mobile' => $mobile,
        'password' => $password, // You should hash this using Hash::make($password)
    ...
    private static function getReferrerId($referralCode)
    {
        if (!$referralCode) return null;

        $user = DB::table('users')->where('referral_code', $referralCode)->first();
...
//         if (!session()->get('otp_verified_' . $mobile)) {
//             return response()->json([
//                 'status' => 401,
//                 'message' => 'Mobile number not verified via OTP'
//             ], 401);
//         }
...
//                 session()->forget('otp_verified_' . $mobile);
//                 return response()->json([
//                     'status' => 200,
...
public function updateAccount(Request $request)
{
    ...
    $account = DB::table('account_details')->where('user_id', $user->id)->first();

    if (!$account) {
        return response()->json([
            'status' => 404,
            'message' => 'Account details not found for this user'
        ], 404);
    }
...
public function withdraw(Request $request)
{
    ...
    $upiid = $user_details->upi_id ?? null;
    if (empty($upiid)) {
        return response()->json([
            'status' => 400,
  ...
  if ($amount >= 500 && $amount <= 25000) {
        ...
        if ($user_recharge == 0) {
            if ($first_recharge == 1) {
                if ($user_wallet >= $amount) {
                    ...
                    $commission = round($amount * 0.05, 2); // 5% commission
                    $final_withdraw_amount = $amount - $commission;

                    $data = DB::table('withdraw_histories')->insert([
                        'user_id' => $userid,
                        'amount' => $final_withdraw_amount, // user will get amount after deducting 5%
                        'commission' => $commission, // (optional: if you want to track it)
                        'account_id' => $accountid,
                        'type' => $type,
                        'order_id' => $orderid,
                        'status' => 1,
                        'typeimage' => "https://root.king11.live/uploads/fastpay_image.png",
                        'created_at' => now(),
                        'updated_at' => now(),
                    ]);

                    // Deduct full amount (before commission) from wallet
                    DB::update("UPDATE `users` SET `wallet` = `wallet` - ?, `winning_wallet` = `winning_wallet` - ? WHERE id = ?", [$amount, $amount, $userid]);
        ...
        return response()->json([
            'status' => 400,
            'message' => 'Minimum Withdraw 500 and Maximum Withdraw 25000'
        ], 200);
    }
}
...
public function profile(Request $request) 
{
    ...
    if ($status == 1) {
        ...
        $responseData = [
            ...
            'apk_link' => "https://root.king77.games/king77.apk",
            //'referral_code_url' => "https://root.globalbet24.club/registerwithref/" . $data->referral_code,
			'referral_code_url' => "https://king77.games/register/" . $data->referral_code,
            'aviator_link' => "https://aviatorudaan.com/",
            'aviator_event_name' => "king77_aviator",
            'wingo_socket_url' => "https://aviatorudaan.com/",
            'wingo_socket_event_name' => "king77wingo",
            'status' => "1",
            'type' => "0"
        ];
...
");
    	...
    	public function attendance_claim(Request $request)
    {
        ...
        // Check first successful recharge code written by akhilesh
        // $firstRecharge = DB::table('payins')
        //     ->where('user_id', $userid)
        //     ->where('status', 2)
        //     ->orderBy('id', 'asc') // first recharge
        //     ->first();
        
        // if (!$firstRecharge || $firstRecharge->cash < 500) {
        //     return response()->json([
        //         'message' => 'Your first deposit is not greater than or equal to ₹500.',
        //         'status' => 400
        //     ], 400);
        // }
...
foreach ($recharges as $recharge) {
    $totalDeposit += $recharge->cash + $recharge->bonus; // bonus add karna hai toh
    if ($totalDeposit >= 500) {
        break;
    }
}
...
if ($totalDeposit < 500) {
    return response()->json([
        'message' => 'Your total deposit amount is not greater than or equal to ₹500.',
        'status' => 400
    ], 400);
}
...
public function activity_rewards(Request $request)
{
    ...
    if (!empty($user)) {
       $usser= DB::table('users')->where('id', $userid)->update([
            ...
            'recharge' => $user->recharge + ($amount * 5), // Add 5x amount to recharge
        ]);
    }else{
     return response()->json([
    				'message' => 'user not found ..!',
...
public function extra_first_deposit(Request $request)
{
    $validator = Validator::make($request->all(), [
        'userid' => 'required|exists:users,id',
        ...
        'extra_fdb_id' => 'required|exists:extra_first_deposit_bonus,id',
    ]);
    ...
    $extra_fdb_id = $request->extra_fdb_id;
    $datetime = now();
    $orderid = date('YmdHis') . rand(10000, 99999);

    // ✅ Validate bonus & cash with extra_first_deposit_bonus table
    $bonusData = DB::table('extra_first_deposit_bonus')->where('id', $extra_fdb_id)->first();

    if (!$bonusData) {
        return response()->json([
            'status' => 400,
            'message' => 'Invalid bonus offer.'
        ]);
    }

    if ($bonusData->first_deposit_ammount != $cash || $bonusData->bonus != $bonus) {
        return response()->json([
            'status' => 400,
            'message' => 'Cash or Bonus amount does not match the bonus offer.'
        ]);
    }
    ...
    $newName = Str::random(10) . '.png';
    $directory = public_path('screenshot_image/');
    ...
    $imageUrl = 'https://root.king77.games/public/screenshot_image/' . $newName;

    // ✅ Insert into payins table
    $insert = DB::table('payins')->insert([
        'user_id' => $userid,
        'cash' => $cash,
        'bonus' => $bonus,
        'transaction_id' => null,
        'type' => 1, // or your custom type code
        'typeimage' => $imageUrl,
        'order_id' => $orderid,
        'status' => 1,
        'extra_fdb_id' => $extra_fdb_id,
        'created_at' => $datetime,
        'updated_at' => $datetime
    ]);
    ...
    if ($insert) {
        return response()->json([
            'status' => 200,
            'message' => 'Extra First Deposit Payin successfully submitted.'
        ]);
    } else {
        return response()->json([
            'status' => 400,
            'message' => 'Failed to process payin.'
        ]);
    }
}
...
public function extra_first_deposit_bonus(Request $request)
{
    ...
    // Step 1: Get all claimed extra_fdb_id for this user from payins + claim
    $claimedBonusIds = DB::table('payins')
        ->join('extra_first_deposit_bonus_claim as claim', function ($join) use ($userid) {
            $join->on('payins.extra_fdb_id', '=', 'claim.extra_fdb_id')
                 ->where('claim.userid', '=', $userid);
        })
        ->whereIn('payins.extra_fdb_id', range(1, 10))
        ->where('payins.status', 2)
        ->pluck('payins.extra_fdb_id')
        ->toArray();

    // Step 2: Fetch all bonus tiers
    $bonusData = DB::table('extra_first_deposit_bonus')
        ->select(
            'id',
            'first_deposit_ammount',
            'tier_name',
            'bonus',
            DB::raw('first_deposit_ammount + bonus as totalamount'),
            'status',
            'created_at'
        )
        ->get();

    // Step 3: Add claimed status
    foreach ($bonusData as &$row) {
        $row->claimed = in_array($row->id, $claimedBonusIds) ? 1 : 0;
    }
    ...
    //             $checkDate = DB::table('extra_first_deposit_bonus')->select('first_deposit_ammount', 'tier_name','bonus', DB::raw('first_deposit_ammount + bonus as totalamount'), 'status','created_at')->get(); 
...
");
    ...
    if (!empty($user)) {
       $usser= DB::table('users')->where('id', $userid)->update([
            'wallet' => $user->wallet + $amount, // Add amount to wallet
            'recharge' => $user->recharge + $amount*5, // Add amount to recharge
        ]);
    }else{
     return response()->json([
    				'message' => 'user not found ..!',
